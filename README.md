...
## Artifact generation for `.rq` files
...
Generate `.ttl` files for all endpoints using base URLs provided in `endpoints.properties`:

```sh
make ttls
```

Then you can compile all `.ttl` files in one for all files in a specific folder:

```sh
make ttl project=UniProt
make ttl project=Bgee
make ttl project=Cellosaurus
make ttl project=Emi
make ttl project=GlyConnect
make ttl project=HAMAP
make ttl project=MetaNetX
make ttl project=neXtProt
make ttl project=OMA
make ttl project=OrthoDB
make ttl project=Rhea
make ttl project=SwissLipids
```

### Generate `.md` files

Generate `.md` files (for the `.ttl` files generated by `make ttls`):
...
Run tests (for the `.ttl` files generated by `make ttls`):
...
## Previous artifact generation and quality assurance
